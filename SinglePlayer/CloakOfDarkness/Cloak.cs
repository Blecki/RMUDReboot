...
*/
            ...
            //    now the bar is dark.
            Perform<MudObject, MudObject>("take").Do((actor, thing) => { Core.GetObject("Bar").SetProperty("ambient light", LightingLevel.Dark); return PerformResult.Continue; });
            ...
            Perform<MudObject, MudObject, MudObject, RelativeLocations>("put")
                ...
                .Do((actor, item, container, relloc) => { Core.GetObject("Bar").SetProperty("ambient light", LightingLevel.Bright); return PerformResult.Continue; });

            //Carry out dropping the cloak in the cloakroom:
            //    now the bar is lit.
            Perform<MudObject, MudObject>("drop")
                .When((actor, item) => actor.Location is Cloakroom)
                .Do((actor, item) => { Core.GetObject("Bar").SetProperty("ambient light", LightingLevel.Bright); return PerformResult.Continue; });
