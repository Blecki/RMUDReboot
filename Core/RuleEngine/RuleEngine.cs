    ...
    public partial class RuleEngine
    {
        ...
        private IEnumerable<RuleSet> EnumerateRuleSets(Object[] Arguments)
        {
            if (Arguments != null)
            {
                ...
                var linkedRuleSourceQueue = new Queue<MudObject>();

                // Loop again, so that all linked rules are enumerated after all parent rules.
                foreach (var arg in Arguments)
                    if (arg is MudObject)
                        if ((arg as MudObject).LinkedRuleSource != null)
                            if (!objectsExamined.Contains((arg as MudObject).LinkedRuleSource)) // This is the mechanism that allows a mud object's location to specify rules.
                            {
                                objectsExamined.Add((arg as MudObject).LinkedRuleSource);
                                linkedRuleSourceQueue.Enqueue((arg as MudObject).LinkedRuleSource);
                            }

                while (linkedRuleSourceQueue.Count != 0)
                {
                    var next = linkedRuleSourceQueue.Dequeue();
                    if (next.Rules != null)
                        yield return next.Rules;

                    if (next.LinkedRuleSource != null && !objectsExamined.Contains(next.LinkedRuleSource))
                    {
                        objectsExamined.Add(next.LinkedRuleSource);
                        linkedRuleSourceQueue.Enqueue(next.LinkedRuleSource);
                    }
                }
            }
        }
